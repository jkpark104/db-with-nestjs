...
import { Ch04DesignFirstModule } from './ch04.module';
...
describe('Ch04 design-first smoke', () => {
  let app: INestApplication;

  beforeAll(async () => {
    const ref = await Test.createTestingModule({
      imports: [Ch04DesignFirstModule],
  ...
  it('GET /products/1 → 200, priceInWon, header spec-derived', async () => {
    const res = await request(app.getHttpServer()).get('/products/1');
    expect(res.status).toBe(200);
    expect(res.body).toHaveProperty('priceInWon');
    expect(res.headers['x-contract-status']).toBe('spec-derived');
  });

  it('GET /users/1 → 200, header spec-derived', async () => {
